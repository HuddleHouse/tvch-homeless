	...
	<div class="col-md-8 col-md-offset-2" data-example-id="striped-table" style="padding-top: 20px;">
			...
			<thead>
		        ...
		        <tr class="active title">
		          ...
		          <th class='column'>Precariously House</th>
...
<?php
	...
	//
	$query = $db->prepare("select coalesce(sum(is_trans), 0) as trans, coalesce(sum(is_unsheltered), 0) as un, coalesce(sum(is_temp), 0) as temp, coalesce(sum(is_hotel), 0) as hotel, coalesce(sum(is_emergency), 0) as emer, count(county) as total from survey_data s
...
<?php
	...
	$query = $db->prepare("select count(p.id) as total, coalesce(sum(s.is_emergency), 0) as emer, coalesce(sum(s.is_hotel), 0) as hotel, coalesce(sum(s.is_temp), 0) as temp, coalesce(sum(s.is_trans), 0) as trans, coalesce(sum(s.is_unsheltered), 0) as un
					from person_data p
						right join survey_data s
							on s.id = p.survey_data_id
						where (select count(pd.id) from person_data pd where pd.age = 2 and pd.is_head = 1 and pd.survey_data_id = s.id) = 1						and (select count(pd.id) from person_data pd where pd.age != 1 and pd.survey_data_id = s.id) > 0 and
	(select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) > 0");
	...
	$query = $db->prepare("select count(p.id) as total, a.description as age, coalesce(sum(s.is_emergency), 0) as emer, coalesce(sum(s.is_hotel), 0) as hotel, coalesce(sum(s.is_temp), 0) as temp, coalesce(sum(s.is_trans), 0) as trans, coalesce(sum(s.is_unsheltered), 0) as un
					from person_data p
						...
						left join agerangelist a
							on a.agerangeid = p.age
						where (select count(pd.id) from person_data pd where pd.age = 2 and pd.is_head = 1 and pd.survey_data_id = s.id) = 1
...
<?php
	...
	//
	$query = $db->prepare("select coalesce(sum(is_trans), 0) as trans, coalesce(sum(is_unsheltered), 0) as un, coalesce(sum(is_temp), 0) as temp, coalesce(sum(is_hotel), 0) as hotel, coalesce(sum(is_emergency), 0) as emer, count(county) as total from survey_data s
	where (select count(pd.id) from person_data pd left join survey_data sd on sd.id = pd.survey_data_id where pd.is_military = 1 and s.id = sd.id) > 0 and (select count(pd.id) from person_data pd where pd.age != 1 and pd.survey_data_id = s.id) > 0 and
	(select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) > 0");
...
<?php
	...
	$query = $db->prepare("select count(p.id) as total, coalesce(sum(s.is_emergency), 0) as emer, coalesce(sum(s.is_hotel), 0) as hotel, coalesce(sum(s.is_temp), 0) as temp, coalesce(sum(s.is_trans), 0) as trans, coalesce(sum(s.is_unsheltered), 0) as un
					from person_data p
						right join survey_data s
							on s.id = p.survey_data_id
						where
						(select count(pd.id) from person_data pd left join survey_data sd on sd.id = pd.survey_data_id where pd.is_military = 1 and s.id = sd.id) > 0
						AND (select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) = 0");
	...
	$query = $db->prepare("select count(p.id) as total, a.description as age, coalesce(sum(s.is_emergency), 0) as emer, coalesce(sum(s.is_hotel), 0) as hotel, coalesce(sum(s.is_temp), 0) as temp, coalesce(sum(s.is_trans), 0) as trans, coalesce(sum(s.is_unsheltered), 0) as un
					from person_data p
						...
						left join agerangelist a
							on a.agerangeid = p.age
						where (select count(pd.id) from person_data pd left join survey_data sd on sd.id = pd.survey_data_id where pd.is_military = 1 and s.id = sd.id) > 0
						AND (select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) = 0
						group by age;");
...
//ethnicity
	...
	from survey_data s
		...
		left join condition_data c
			on c.person_data_id = p.id
		where (select count(pd.id) from person_data pd where pd.is_head = 1 and (pd.history_1 = 1
