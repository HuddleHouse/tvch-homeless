	...
	from survey_data s
		where (select count(pd.id)
			from person_data pd
				...
				and (pd.age = 2 or pd.age = 3)) = 0 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetch();
...
<?php
	...
	(select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) > 0 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetch();
...
<?php
	...
	from survey_data s
		...
		where (select count(pd.id) from person_data pd where pd.age = 1 and pd.survey_data_id = s.id) = 0 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetch();
...
//ethnicity
	...
	from survey_data s
		...
		left join condition_data c
			on c.person_data_id = p.id
		where (select count(pd.id) from person_data pd where pd.is_head = 1 and (pd.history = 'option1' or pd.history = 'option2') and pd.survey_data_id = s.id and (select count(cd.id) from condition_data cd where cd.person_data_id = pd.id) > 0) = 1 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetchAll();
	...
	from person_data p
		left join survey_data s
			on s.id = p.survey_data_id
		where p.is_military = 1 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetchAll();
	...
	from person_data p
		...
		and g.genderid = 2 and county = :county;");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetchAll();
	...
	from person_data p
		...
		left join condition_data c
			on c.person_data_id = p.id
		where c.cond = 'severe-mental' and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$c = $query->fetch();
	...
	from person_data p
		...
		left join condition_data c
			on c.person_data_id = p.id
		where c.cond = 'severe-chronic' and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$c = $query->fetch();
	...
	from person_data p
		...
		left join condition_data c
			on c.person_data_id = p.id
		where c.cond = 'hiv' and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$count = $query->fetchAll();
...
$query = $db->prepare("select count(p.id) as total, coalesce(sum(s.is_emergency), 0) as emer, coalesce(sum(s.is_hotel), 0) as hotel, coalesce(sum(s.is_temp), 0) as temp, coalesce(sum(s.is_trans), 0) as trans, coalesce(sum(s.is_unsheltered), 0) as un
	from person_data p
		left join survey_data s
			on s.id = p.survey_data_id
		where p.is_violence = 1 and county = :county");
	$query->bindParam(':county', $county, PDO::PARAM_STR);
	$query->execute();
	$c = $query->fetch();
